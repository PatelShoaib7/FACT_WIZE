...
import { Accordion, AccordionButton, AccordionIcon, AccordionItem, AccordionPanel, Avatar,Box ,
    EditableInput,
    EditableTextarea,
    EditablePreview,
    ButtonGroup,
...
export const Home = () => {
    ...
    const getData = async()=>{
        ...
        let Comigdata = await res.json();
         ...
         const updateDATA = Comigdata.map((elem)=>
       {
            let today = new Date();
            let birthDate = new Date(elem.dob);
  ...
  const  handleChange =(e)=>{
      ...
      const upDated = data.filter((el)=> el.first[0] || el.last[0] == k)
      setSearch(upDated)
  ...
  return (
    <div><h1> </h1>
         ...
         <AccordionItem border="1px solid lightgrey" borderRadius={'6px'} >
    {({ isExpanded }) => (
      <>
        <h2>
          <AccordionButton>
            <Box display={'flex'} flex='1' textAlign='left'>
                    <Box p='1'>
                             {elem.id} <Avatar size='md' name='Kent Dodds' src={index%2==0 ? 'https://bit.ly/kent-c-dodds' : 'https://bit.ly/prosper-baba'} />
                    </Box>
                <br />
                    <Box mt='4' fontSize={'18px'} display="flex"> {elem.first} {'  '}{elem.last}
                      <br />
                      <p style={{color:'lightgrey' ,fontSize:'16px' , marginLeft:'5px' ,marginTop:'2px'}}> age : {elem.dob}</p> 
        ...
        <AccordionPanel pb={4}>
          <Box border="1px slid black" height="40px">
          <Stack direction='row' spacing='2' justifyContent='' alignItems={'center'} color={'grey'}>
                  <Box  h='30px' w="6rem" border="1px solid lightgrey" borderRadius={'12px'}>
                    Age :{elem.dob}
                  </Box>
                  <Box h='30px' w="8rem" border="1px solid lightgrey" borderRadius={'12px'}>
                    Gender : {elem.gender}
                  </Box>
                  <Box  w="auto" h='30px'border="1px solid lightgrey" borderRadius={'12px'}>
                   Country :{elem.country}
                  </Box>
                </Stack>
          </Box>
        {elem.description}
        <Button value={elem.id} onClick={ handleDelete} colorScheme={'red'}>Delete</Button> {'   '}
      
        <Button value={elem.id} onClick={handleEDIT}>Edit</Button>
        </AccordionPanel>
      </>
    )}
  </AccordionItem>
